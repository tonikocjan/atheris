...
\indent Delovanje programskega jezika Atheris je preverjeno na testnih primerih, izmerjena pa je tudi hitrost samega izvajanja in primerjana z osnovnim jezikom PINS ter za referenco tudi s Pythonom.
...
V zahtevnejših prevajalnikih se v fazi semantične analize opravi še veliko drugih stvari. Običajno so to lahko analiza inicializiranosti spremenljivk, zaznavanje mrtve kode, ipd., vendar se s temi rečmi tekom diplomske naloge ne ukvarjamo.
...
Klicni zapis je prostor v pomnilniku, ki je dodeljen vsakemu posameznemu klicu funkcije posebej, in vsebuje vse potrebne informacije za klicanje in izvedbo funkcij. Običajno klicni zapis vsebuje prostor za lokalne spremenljivke, vhodne parametre in register, kamor se shrani izhodna vrednost funkcije. \\
...
\section{Quicksort s polimorfnimi operacijami}
...
Po pričakovanjih se izkaže, da je izvajanje res občutno počasnejše; in sicer kar cca. 400\%. Algoritem sem poiskusil pohitriti tako, da sem se izognil operaciji \textit{instance of}. Izvajanje se je s to optimizacijo pohitrilo za približno 10 sekund.
